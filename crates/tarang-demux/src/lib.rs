...
mod tests {
    ...
    fn integration_wav_probe_read_seek_read() {
        ...
        let config = MuxConfig {
            codec: tarang_core::AudioCodec::Pcm,
            sample_rate: 44100,
            channels: 2,
            bits_per_sample: 16,
        };
    ...
    fn integration_ogg_mux_demux_roundtrip() {
        ...
        let mut buf = Cursor::new(Vec::new());
        let config = MuxConfig {
            codec: tarang_core::AudioCodec::Opus,
            sample_rate: 48000,
            channels: 2,
            bits_per_sample: 16,
        };
        ...
        assert_eq!(audio[0].codec, tarang_core::AudioCodec::Opus);
    ...
    fn integration_ogg_vorbis_mux_demux_roundtrip() {
        ...
        let mut buf = Cursor::new(Vec::new());
        let config = MuxConfig {
            codec: tarang_core::AudioCodec::Vorbis,
            sample_rate: 44100,
            channels: 1,
            bits_per_sample: 16,
        };
        ...
        assert_eq!(info.format, ContainerFormat::Ogg);
        let audio = info.audio_streams().collect::<Vec<_>>();
        assert_eq!(audio[0].codec, tarang_core::AudioCodec::Vorbis);
